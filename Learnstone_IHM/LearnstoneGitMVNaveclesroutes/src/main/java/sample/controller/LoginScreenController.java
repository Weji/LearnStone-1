...
public class LoginScreenController implements Initializable{
    ...
    public void initialize (URL location, ResourceBundle resources) {
        ...
        if(!(passwordFieldPassword == null)){
            ...
            prepareCreationPersonButton();
        }
    }

    private void prepareCreationPersonButton() {
        textFieldUserNameCreation.setOnKeyReleased(event -> {
            if(event.getCode() == KeyCode.ENTER){
            }
        });
    }
...
//        System.out.println(loginScene);
    ...
    @FXML private void ui(){
        mediaBackGroundSound = new Media((getClass().getClassLoader().getResource("song/OUIIzerator.mp3").toExternalForm()));
...
//        System.out.println(getClass().getResource("../resource/fxml_page/LoginScreenPageSubscription.fxml"));
    ...
    @FXML private void connectionAuthentication() throws IOException {
        stage = (Stage) labelDayDate.getScene().getWindow();
            try{
                int id_User = webService.getIdPerson(textFieldAccount.getText(), passwordFieldPassword.getText());
                if(id_User > 0){
//                    System.out.println("connectionAuthentication : " + getClass().getClassLoader().getResource("fxml_page/WelcomePage.fxml") );
                    ...
                    welcomePageController.prepareData(id_User);
