...
//                }
    ...
    public void checkPlugin(VBox vBox) throws IllegalAccessException, InstantiationException {
        if(getClass().getClassLoader().getResource("plugins/PluginWebView.jar") != null){
            plugin = new Plugin(getClass().getClassLoader().getResource("plugins/PluginWebView.jar").toString());
            plugin.load();
            c = plugin.loadStartupClass();
